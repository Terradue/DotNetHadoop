...
namespace Terradue.Hadoop.WebHDFS
{
	...
	public class WebHDFSClient
	{
		...
		{
			JsonObject files = GetWebHDFS(path, "LISTSTATUS");
		...
		{
			JsonObject file = GetWebHDFS(path, "GETFILESTATUS");
			return new DirectoryEntry(file.Get<JsonObject>("FileStatus"));
		}
		...
		public string GetHomeDirectory()
		{
			...
			{
				 JsonObject path =   GetWebHDFS("/", "GETHOMEDIRECTORY");
				this.homeDirectory = path.Get<string>("Path");
		...
		{
			 JsonObject contentSummary =   GetWebHDFS(path, "GETCONTENTSUMMARY");
			return new ContentSummary(contentSummary.Get< JsonObject>("ContentSummary"));
		}
		...
		{
			 JsonObject fileChecksum =   GetWebHDFS(path, "GETFILECHECKSUM");
			return new FileChecksum(fileChecksum.Get< JsonObject>("FileChecksum"));
		}
		...
		public   string CreateFile(Stream content, string remotePath, bool overwrite = false)
		{
            try {
                ...
                throw new RemoteException(JsonObject.Parse(err).Get<JsonObject>("RemoteException")).GetException();
		...
		public    string AppendFile(string localFile, string remotePath)
		{
			...
			BufferedStream sc = new BufferedStream(System.IO.File.OpenRead(localFile));
			hc.UploadData (postLocation, "POST", sc.ReadFully());
		...
		public   string AppendFile(Stream content, string remotePath)
		{
			...
			var postLocation = hc.Headers[HttpResponseHeader.Location].ToString();
			BufferedStream sc = new BufferedStream(content);
			hc.UploadData (postLocation, "POST", sc.ReadFully());
			return hc.Headers[HttpResponseHeader.Location].ToString();
		}
		...
		private JsonObject GetWebHDFS(string path, string operation)
		{
			WebClient hc = this.CreateHTTPClient();
			string uri = this.GetUriForOperation(path);
			uri += "op=" + operation;
			var resp =   hc.DownloadString (uri);
			JsonObject files =  JsonObject.Parse(resp);
			return files;
		}
	}
